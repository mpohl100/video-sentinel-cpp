...
const auto get_test_slices(const math2d::Point &start, const math2d::Point &end,
                           const math2d::Point &imageDimensions = math2d::Point{
                               0, 0}) {
  if (start.y > end.y) {
  ...
  auto slices = od::Slices{start, imageDimensions};
...
TEST_CASE("Object", "[object]") {
  ...
  SECTION("SlicesRotateClockwiseWorks") {
    const auto imageDimensions = math2d::Point{500, 1000};
    auto object_to_rotate = std::make_shared<od::Object>(get_test_slices(
        math2d::Point{0, 0}, math2d::Point{50, 100}, imageDimensions));
    const auto target_object = std::make_shared<od::Object>(
        get_test_slices(math2d::Point{450, 0}, math2d::Point{500, 100},
                        math2d::Point{imageDimensions.y, imageDimensions.x}));
    object_to_rotate->slices.rotate_clockwise();
    CHECK(object_to_rotate->slices == target_object->slices);
  }
  SECTION("SlicesRotateCounterClockwiseWorks") {
    const auto imageDimensions = math2d::Point{500, 1000};
    auto object_to_rotate = std::make_shared<od::Object>(get_test_slices(
        math2d::Point{0, 0}, math2d::Point{50, 100}, imageDimensions));
    const auto target_object = std::make_shared<od::Object>(
        get_test_slices(math2d::Point{0, 900}, math2d::Point{50, 1000},
                        math2d::Point{imageDimensions.y, imageDimensions.x}));
    object_to_rotate->slices.rotate_counterclockwise();
    CHECK(object_to_rotate->slices == target_object->slices);
  }
}
